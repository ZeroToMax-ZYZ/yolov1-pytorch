...
class YoloLoss(nn.Module):
    def __init__(self, bs, S, B, C, lambda_coord, lambda_noobj, ic_debug=False):
        super().__init__()
        self.bs, self.S, self.B, self.C = bs, S, B, C
        self.lambda_coord, self.lambda_noobj = lambda_coord, lambda_noobj
        self.ic_debug = ic_debug

    def _grid_mesh(self, x):
        device = x.device   
        x = torch.arange(self.S)
        y = torch.arange(self.S)
        # 行 列
        I, J = torch.meshgrid(x, y, indexing='ij')
        # ic(I, J)
        I = I.reshape(1, self.S, self.S, 1).expand(self.bs, self.S, self.S, 1).to(device)
        J = J.reshape(1, self.S, self.S, 1).expand(self.bs, self.S, self.S, 1).to(device)
        return I, J
    ...
    def _iou_xyxy(self, pred_x1, pred_y1, pred_x2, pred_y2, gt_x1, gt_y1, gt_x2, gt_y2):
        ...
        gt_x1: (2, 7, 7, 1)
        '''
        roi_x1 = torch.min(pred_x1, gt_x1)
        roi_y1 = torch.min(pred_y1, gt_y1)
        roi_x2 = torch.max(pred_x2, gt_x2)
        roi_y2 = torch.max(pred_y2, gt_y2)
    ...
    def forward(self, pred, target):
        ...
        gt_cls = target[:, :, :, 5:6] # ([2, 7, 7, 1])
        # pred : (2, 7, 7, 11)
        pred_2xywhc = pred[:, :, :, :10] # ([2, 7, 7, 10])
        pred_cls = pred[:, :, :, 10:11] # ([2, 7, 7, 1])

        pred_xywhc = pred_2xywhc.reshape(self.bs, self.S, self.S, 2, 5)
        ...
        # 有物体的grid cell的mask
        grid_mask = gt_conf.expand(self.bs, self.S, self.S, 2) # ([2, 7, 7, 1]) --> ([2, 7, 7, 2])
        # 找到有物体的grid cell，同时bbox负责的mask
        grid_bbox_mask = gt_conf * iou_index_mask

        # 对齐维度
        gt_x = gt_x.expand(self.bs, self.S, self.S, 2)
        gt_y = gt_y.expand(self.bs, self.S, self.S, 2)
        gt_w = gt_w.expand(self.bs, self.S, self.S, 2)
        gt_h = gt_h.expand(self.bs, self.S, self.S, 2)
        gt_conf = gt_conf.expand(self.bs, self.S, self.S, 2)

        iou_max = iou_max.reshape(self.bs, self.S, self.S, 1).expand(self.bs, self.S, self.S, 2)
        ...
        # 第五行： 类别损失
        loss_classes = grid_bbox_mask * (pred_cls - gt_cls)**2
...
if __name__ == "__main__":
    ...
    loss = YoloLoss(bs=2, S=7, B=2, C=1, lambda_coord=5, lambda_noobj=0.5, ic_debug=True)
